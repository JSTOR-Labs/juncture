...
baseurl = 'https://iiif-v2.visual-essays.app'
cors_headers = {
...
# ingest_endpoint_alt = 'https://iiif-server-atjcn6za6q-uc.a.run.app/api/v1/ingest'
ingest_endpoint_alt = 'https://cdn.visual-essays.app/api/v1/ingest'

ingest_endpoint = ingest_endpoint_iiifhosting

with open(f'{SCRIPT_DIR}/iiifhosting-creds', 'r') as fp:
    iiifhosting_user, iiifhosting_token = fp.read().strip().split(':')

mongo_host = 'cluster0.llrbv.gcp.mongodb.net'
with open(f'{SCRIPT_DIR}/atlas-creds', 'r') as fp:
    atlas_user, atlas_password = fp.read().strip().split(':')
atlas_endpoint = f'mongodb+srv://{atlas_user}:{atlas_password}@{mongo_host}/?retryWrites=true&w=majority'
...
def get_image_data(mdb, url, service=None, manifest=None):
    if service not in ('alt', 'primary'):
        service = 'alt' if ingest_endpoint == ingest_endpoint_alt else 'primary'
    if service == 'alt':
        image_data = mdb['images-alt'].find_one({'_id': url})
        return image_data if image_data else make_iiif_image_alt(mdb, url, manifest)
    else:
        return mdb['images'].find_one({'_id': url})

def make_iiif_image(mdb, manifest=None, **kwargs):
    if ingest_endpoint == ingest_endpoint_alt:
        url = kwargs['url']
        id = sha256(url.encode('utf-8')).hexdigest()
        make_iiif_image_alt(mdb, url, manifest)
    else:
...
def update_manifests_with_image_data(mdb, image_data):
    image_data['url'] = image_data['url'].replace('http:', 'https:')
    # _filter = {'items.0.items.0.id': {'$eq': image_data['external_id']}} # manifest v3
    ...
    if request.method == 'OPTIONS':
        ...
        if manifest:
            ...
            if request.method == 'GET':
                ...
                '''
                image_data = get_image_data(mdb, source_url, service=service, manifest=manifest)
                if image_data and 'url' in image_data:

                    if 'service' in manifest['sequences'][0]['canvases'][0]['images'][0]:
                        manifest['sequences'][0]['canvases'][0]['images'][0]['resource']['service']['@id'] = image_data['url'][:-1].replace('iiif-server-atjcn6za6q-uc.a.run.app', 'cdn.visual-essays.app')
                    else:
                        manifest['sequences'][0]['canvases'][0]['images'][0]['resource'] = {
                            '@id': image_data['external_id'],
                            '@type': 'dctypes:Image',
                            ...
                            'service': {
                                '@context': 'http://iiif.io/api/image/2/context.json',
                                '@id': image_data['url'][:-1].replace('iiif-server-atjcn6za6q-uc.a.run.app', 'cdn.visual-essays.app').replace('http:','https:'),
                                'profile': 'https://iiif.io/api/image/2/level2.json'
                            }
                        }
                        for attr in ('height', 'width'):
                            if attr in image_data:
                                manifest['sequences'][0]['canvases'][0]['images'][0]['resource'][attr] = image_data[attr]
                    manifest['thumbnail'] = f'{image_data["url"]}full/150,/0/default.jpg'.replace('iiif-server-atjcn6za6q-uc.a.run.app', 'cdn.visual-essays.app').replace('http:','https:')
                    '''
        ...
        return manifest, 200, 200, cors_headers
